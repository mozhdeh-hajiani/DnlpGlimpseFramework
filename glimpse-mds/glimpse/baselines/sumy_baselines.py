...
def main():
    ...
    for N in [1]:  
        ...
        print("✅ Sample summaries generated:")
        print(dataset[["text", "summary"]].head(5))  # Print first 5 summaries

        name = f"{Path(args.dataset).stem}-_-{args.method}-_-sumy_{N}.csv"
        path = Path(args.output) / name

        Path(args.output).mkdir(exist_ok=True, parents=True)
        dataset.to_csv(path, index=False)

        print(f"✅ Summaries saved at: {path}")  # Print save path
