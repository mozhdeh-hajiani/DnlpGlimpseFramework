...
# 🚀 Define PEGASUS Decoding Configurations
GENERATION_CONFIGS = {
    "beam_search": {
        "max_new_tokens": 256,  # PEGASUS supports shorter summaries
        "do_sample": False,
        "num_beams": 4,
        "early_stopping": True,
        "length_penalty": 1.0,
...
def parse_args():
    ...
    parser.add_argument("--batch_size", type=int, default=8)  # 🚀 PEGASUS is memory-heavy
    parser.add_argument("--device", type=str, default="cuda")
    parser.add_argument("--trimming", action=argparse.BooleanOptionalAction, default=True)
    parser.add_argument("--output_dir", type=str, default="data/pegasus_candidates")
...
def evaluate_summarizer(model, tokenizer, dataset: Dataset, decoding_config, batch_size: int, device: str, trimming: bool) -> Dataset:
    ...
    for batch in tqdm(dataloader):
        ...
        inputs = tokenizer(
            text,
            max_length=1024,  # ✅ PEGASUS handles full documents
            padding="longest",  # ✅ PEGASUS performs best with this
            truncation=True,
            return_tensors="pt",
        )
        ...
        # ✅ Fix "max_new_tokens" duplication issue
        decoding_config = {k: v for k, v in decoding_config.items() if k != "max_new_tokens"}

        outputs = model.generate(
            **inputs,
            max_new_tokens=GENERATION_CONFIGS["beam_search"]["max_new_tokens"],  # ✅ Explicitly set max output length
            decoder_start_token_id=tokenizer.pad_token_id,  # ✅ PEGASUS fix
            **decoding_config,
        )
...
def main():
    ...
    # ✅ PEGASUS fix: Ensure PAD token exists
    if tokenizer.pad_token is None:
        tokenizer.pad_token = tokenizer.eos_token  # Use EOS token as PAD for PEGASUS
